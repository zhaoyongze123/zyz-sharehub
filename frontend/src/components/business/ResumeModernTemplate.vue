  ...
  <div class="resume-physical-page bg-white mx-auto shadow-sm" ref="resumeRef">
    <div class="resume-wrapper">
      ...
      >
        ...
        <div v-if="module.type === 'basic' && module.visible" class="bg-[#1a7db1] text-white -mx-12 -mt-10 px-12 pt-10 pb-6 mb-8 flex justify-between relative resume-module-block hover:outline-dashed hover:outline-2 hover:outline-blue-400 group cursor-pointer transition-all" @click.stop="$emit('focus-field', module.id, '')">
          <div class="flex-1">
            <h1 class="text-3xl font-extrabold text-white tracking-wider mb-2 flex items-center gap-2">
              <span v-if="module.data.nameVisible !== false">{{ module.data.name }}</span>
              <span v-if="module.data.intentVisible !== false" class="text-lg font-medium text-white/80 tracking-normal ml-3 px-2 border-l-2 border-gray-300">{{ module.data.intent }}</span>
            </h1>
            <div class="flex flex-wrap gap-x-4 gap-y-1 text-sm text-blue-100 mt-3 font-medium">
               <div v-if="module.data.phoneVisible !== false" class="flex items-center hover:bg-white/10 px-1 rounded transition-colors" @click.stop="$emit('focus-field', module.id, 'input_basic_phone')">{{ module.data.phone }}</div>
               <div v-if="module.data.emailVisible !== false" class="flex items-center hover:bg-white/10 px-1 rounded transition-colors" @click.stop="$emit('focus-field', module.id, 'input_basic_email')">{{ module.data.email }}</div>
               <div v-if="module.data.ageVisible !== false" class="flex items-center hover:bg-white/10 px-1 rounded transition-colors" @click.stop="$emit('focus-field', module.id, 'input_basic_age')">{{ module.data.age }}岁</div>
               <div v-if="module.data.genderVisible !== false" class="flex items-center hover:bg-white/10 px-1 rounded transition-colors" @click.stop="$emit('focus-field', module.id, 'input_basic_gender')">{{ module.data.gender }}</div>
               <div v-if="module.data.currentCityVisible !== false" class="flex items-center hover:bg-white/10 px-1 rounded transition-colors" @click.stop="$emit('focus-field', module.id, 'input_basic_currentCity')">现居{{ module.data.currentCity }}</div>
               <div v-if="module.data.experienceVisible !== false" class="flex items-center hover:bg-white/10 px-1 rounded transition-colors" @click.stop="$emit('focus-field', module.id, 'input_basic_experience')">{{ module.data.experience }}</div>
            </div>
            <div class="flex flex-wrap gap-x-4 gap-y-1 text-sm text-blue-100 mt-1 font-medium">
               <div v-if="module.data.cityVisible !== false" class="flex items-center hover:bg-white/10 px-1 rounded transition-colors" @click.stop="$emit('focus-field', module.id, 'input_basic_city')">意向城市：{{ module.data.city }}</div>
               <div v-if="module.data.salaryVisible !== false" class="flex items-center hover:bg-white/10 px-1 rounded transition-colors" @click.stop="$emit('focus-field', module.id, 'input_basic_salary')">期望薪资：{{ module.data.salary }}</div>
               <div v-if="module.data.availabilityVisible !== false" class="flex items-center hover:bg-white/10 px-1 rounded transition-colors" @click.stop="$emit('focus-field', module.id, 'input_basic_availability')">到岗情况：{{ module.data.availability }}</div>
            </div>
          </div>
        ...
        <div v-if="(module.type === 'education' || module.type === 'experience' || module.type === 'project') && module.visible" class="mb-2 pt-5 pl-[120px] pb-4 relative border-t border-gray-200 resume-module-block hover:outline-dashed hover:outline-2 hover:outline-blue-400 group cursor-pointer transition-all" @click.stop="$emit('focus-field', module.id, '')">
          <!-- Section Title -->
          <div class="w-[120px] shrink-0 text-right pr-6 absolute left-0 top-0 h-full border-t border-gray-200 pt-5"><h2 class="text-lg font-bold text-[#1a7db1] tracking-wide">{{ module.title }}</h2></div>
        ...
        <!-- Skills Module -->
        <div v-if="module.type === 'skills' && module.visible" class="mb-2 pt-5 pl-[120px] pb-4 relative border-t border-gray-200 resume-module-block hover:outline-dashed hover:outline-2 hover:outline-blue-400 group cursor-pointer transition-all" @click.stop="$emit('focus-field', module.id, '')">
          <div class="w-[120px] shrink-0 text-right pr-6 absolute left-0 top-0 h-full border-t border-gray-200 pt-5"><h2 class="text-lg font-bold text-[#1a7db1] tracking-wide">{{ module.title }}</h2></div>
        ...
        <div v-if="module.type === 'text' && module.visible" class="mb-2 pt-5 pl-[120px] pb-4 relative border-t border-gray-200 resume-module-block hover:outline-dashed hover:outline-2 hover:outline-blue-400 group cursor-pointer transition-all" @click.stop="$emit('focus-field', module.id, 'input_text_content')">
          <div class="w-[120px] shrink-0 text-right pr-6 absolute left-0 top-0 h-full border-t border-gray-200 pt-5"><h2 class="text-lg font-bold text-[#1a7db1] tracking-wide">{{ module.title }}</h2></div>
        ...
        <!-- Text List Module -->
        <div v-if="module.type === 'textList' && module.visible" class="mb-2 pt-5 pl-[120px] pb-4 relative border-t border-gray-200 resume-module-block hover:outline-dashed hover:outline-2 hover:outline-blue-400 group cursor-pointer transition-all" @click.stop="$emit('focus-field', module.id, '')">
          <div class="w-[120px] shrink-0 text-right pr-6 absolute left-0 top-0 h-full border-t border-gray-200 pt-5"><h2 class="text-lg font-bold text-[#1a7db1] tracking-wide">{{ module.title }}</h2></div>
