...
ADMIN_SMOKE_SCRIPT_EXIT_CODE=1
FAILURE_REASON=""
...
ADMIN_SMOKE_SCRIPT_EXIT_CODE=${ADMIN_SMOKE_SCRIPT_EXIT_CODE}
...
if [[ ${ADMIN_SMOKE_SCRIPT_EXIT_CODE} -ne 0 ]]; then
  record_failure "Playwright smoke 失败，退出码=${ADMIN_SMOKE_SCRIPT_EXIT_CODE}"
  exit "${ADMIN_SMOKE_SCRIPT_EXIT_CODE}"
fi
